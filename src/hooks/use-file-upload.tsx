  ...
  const handleFileUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (invalidFiles.length > 0) {
      toast({
        ...
        description:
          "Please upload a JPEG, PNG, GIF, WEBP image or a PDF.",
        variant: "destructive",
      });
    }
    if (duplicateFiles.length > 0) {
      toast({
        ...
        description: `${duplicateFiles.map((f) => f.name).join(", ")} already uploaded.`,
        variant: "destructive",
      });
    }
  ...
  const handlePaste = async (
    ...
    if (invalidFiles.length > 0) {
      toast({
        title: "Invalid file type",
        description:
          "Please paste a JPEG, PNG, GIF, WEBP image or a PDF.",
        variant: "destructive",
      });
    }
    if (duplicateFiles.length > 0) {
